...
export function Nav() {
  ...
  const functionClick = ((index) => {
    index == 0 ? (navigate("/home")) : index == 1 ? (window.location.href = "https://www.1337leets.com/calculator"):   (index == 2 ? (window.location.href = "https://www.1337leets.com/PeerFinder") : (<></>));
  })
  const object = [{bars: 'Rank'}, {bars: 'Xp-Cal'}, {bars: 'Peer-Fi'}];
    ...
    return ( 
      ...
      <div className=" flex items-center justify-center flex-row rounded-[5px] h-[95%] w-[25%]">
        <div style={{boxShadow: '0px 0px 10px yellow'}}  className="cursor-pointer w-[50px] h-[50px] border-solid border-yellow-200  border-[2px] rounded-[50%]">
          ...
          {visible &&
              <div onClick={() => functionLogout()} className="absolute z-50 cursor-pointer flex top-[66px] font-bars2 items-center justify-center  w-[100px] h-[30px] rounded-[20px] bg-white duration-300 hover:bg-slate-300">
